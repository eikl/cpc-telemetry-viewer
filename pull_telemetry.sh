...
# flock ships by default on Linux (util-linux) but not on macOS (it's a
# Homebrew package there); launchd/cron/systemd can also hand this script a
# minimal PATH that omits wherever it lives, and a missing command looks
# identical to a busy lock to `if ! flock ...; then` (both non-zero exit),
# silently turning every run into a same-as-before no-op. Widen PATH to cover
# the common install locations on both platforms and fail loudly instead of
# guessing.
